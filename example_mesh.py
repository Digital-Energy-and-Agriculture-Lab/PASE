...
"""
Example script dedicated to the Mesh class.

This script follows the standard example structure while showcasing the updated
Mesh API:
- creation of rectangular surfaces with triangular or rectangular faces,
- oriented ground meshes driven by an azimuth,
- import of PV polydata surfaces with automatic filtering and refinement,
- addition of small triangular probes,
- mesh metadata tracking and data extraction helpers.

The goal is to keep the workflow explicit and readable for end users.
"""
...
PV_module_1 = YAML_Inputs_provider(file='Example1_PV_Module.yaml',
                                   subpath=os.path.join('HARDWARE', 'PV_MODULES')).inputs
Structure = YAML_Inputs_provider(file='Example1_PV_structure.yaml',
                                 subpath=os.path.join('HARDWARE', 'STRUCTURES')).inputs
crop_config = YAML_Inputs_provider(file='simple_example.yml',
                                   subpath=os.path.join('CROPS', 'config')).inputs
...
# Add a tilted sampling surface above the field
tilted_surface_id = M.add_rectangular_surface(
    width=6.0,
    height=3.0,
    density=0.5,
    name="tilted_surface",
...
# Add a triangular probe close to the PV structure
probe_id = M.add_triangular_probe(
...
# Import PV surfaces as sampling meshes with the updated helper
# Filter of the PV using a property dict
imported_pv_ids = M.import_from_pvconfiguration(
    pv_objects=PV_1_3Dconfig.PV_central_MB,
    name="pv_surface",
    include_top=True,
    include_bottom=False,
    density=0.5,
    property_dict={"Block_X" : [0],"Block_Y": [0]}
)
...
L.get_daily_irradiation_map(Sun_positions_samp.SP, Light_instance.data,
                            visualization=True,
                            year=Loc_1['SimulationStartingYear'],
                            julian_day=5)
...
PV_central.get_several_years_of_electricity_production(Sun_positions_complete,
                                                       Light_instance.data,
                                                       WD.nyears_data)
