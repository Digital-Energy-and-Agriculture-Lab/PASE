...
def ET0_FAO56_PM():
    ...
    avg_T_K = self.avg_T+273.16
    max_temp_K = self.max_temp+273.16
    min_temp_K = self.min_temp+273.16

    # Computation of all the data needed to compute reference evapotranspiration (ETo) (link in first doc string)

    slope_vap_pressure_curve = (4098*0.6108*math.exp((17.27*self.avg_T)/(self.avg_T+237.3)))/(self.avg_T+237.3)**2

    atmospheric_pressure = 101.3*((293-(0.0065*self.altitude))/293)**5.26
    psychometric_constant = (specific_heat*atmospheric_pressure)/(latent_heat_vaporization*ratio_molecul_weight_watervap_dryair)

    mean_sat_vap_pressure = (0.6108*(math.exp((17.27*self.max_temp)/(self.max_temp+237.3))+math.exp((17.27*self.min_temp)/(self.min_temp+237.3))))/2
    if self.tmy == 1 :
        actual_vap_pressure = (((0.6108*(math.exp((17.27*self.min_temp)/(self.min_temp+237.3))))*(self.max_RH/100))+((0.6108*(math.exp((17.27*self.max_temp)/(self.max_temp+237.3))))*(self.min_RH/100)))/2
    elif self.tmy == 0 :
        actual_vap_pressure = self.vap_press/10
    ...
    inverse_rel_dist_earth_sun = 1+(0.033*math.cos((2*math.pi*self.day_of_the_year)/self.n_days_in_year))
    solar_decination = 0.409*math.sin(((2*math.pi*self.day_of_the_year)/self.n_days_in_year)-1.39)
    sunset_hour_angle = math.acos(-math.tan(self.latitude*(math.pi/180))*math.tan(solar_decination))

    extraterre_radiation = ((24*60)/math.pi)*solar_constant*inverse_rel_dist_earth_sun*(sunset_hour_angle*math.sin(self.latitude*(math.pi/180))*math.sin(solar_decination)+math.cos(self.latitude*(math.pi/180))*math.cos(solar_decination)*math.sin(sunset_hour_angle))

    clear_sky_solar_radiation = (0.75+((2*10**-5))*self.altitude)*extraterre_radiation
