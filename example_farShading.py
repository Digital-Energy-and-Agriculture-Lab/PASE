...
# Instantiation of the Horizon object
# Fetch horizon profile from PVGIS (REQUIRES INTERNET CONNECTION)
print(f"Fetching horizon data from PVGIS for Lat: {lat}, Lon: {lon}...")
horizon = Horizon(lat, lon)
try:
    horizon.get_horizon_pvgis()
    print("Horizon data fetched successfully.")
    # Optional: print some horizon points to verify
    # print(horizon.horizon_profile.head())
except Exception as e:
    print(f"Failed to fetch horizon data: {e}. Proceeding without horizon masking.")
    horizon = None
